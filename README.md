...
>
> **Python and TypeScript are partially enforced** via the F020 profile architecture. The two hooks shipped so far (`verify-green` for Stop events, `check-test-exists` for Edit/Write events) dispatch by profile and run the right tooling per language. The other three hooks (check-code-quality, check-seam-evidence, check-completion-discipline) and tier-0 standards Python-vocabulary purge are queued under task #23.
>
> Concrete state per stack:
>
> - **Python:** `verify-green` runs `uv run pytest --cov` + `mypy` + `ruff check` (against scripts/ + hooks/helpers/ — honest scope, not whole-repo). `check-test-exists` blocks edits to `src/*.py` without a sibling `tests/test_<module>.py`. Backend-developer / code-reviewer / code-simplifier agents already work.
> - **TypeScript:** `verify-green` runs `npm test` + `tsc --noEmit` + `eslint` (skipping each step cleanly when its config is absent). `check-test-exists` blocks edits to `src/*.ts` and `src/*.tsx` without a sibling `*.test.ts` / `*.spec.ts`. Frontend-developer + frontend-dashboard-refactorer agents are TS-coded.
> - **Go, Rust, Java, Swift, Ruby, Terraform, k8s, markdown:** the profile *architecture* supports them, but no per-language profile content has been authored yet. Operators on these stacks get `/init-project` + DOMAIN.md/PROJECT.md authoring + the agent layer; F020 hooks WARN-skip with a clear stderr line on every unmatched file (no silent gaps).
>
> Trace: see the F020 + F021 rows in the F-feature table below. The audit at `docs/audits/F020-language-coupling-audit.md` lists the 24 surfaces being migrated, the six ADRs at `docs/adrs/F020-001..006` document the load-bearing decisions, and the F021 test fixture at `tests/fixtures/typescript-repo/` proves the architecture works on a second language.
