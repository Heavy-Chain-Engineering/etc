...
> **Language support — honest status (2026-05-18).**
>
> **Python, TypeScript, Go, and Rust are partially enforced** via the F020 profile architecture. Three hooks now dispatch by profile and run the right tooling per language: `verify-green` (Stop events), `check-test-exists` (Edit/Write events), and `check-code-quality` (Edit/Write events). The other two hooks (`check-seam-evidence`, `check-completion-discipline`) and tier-0 standards Python-vocabulary purge are queued under task #23.
>
> Concrete state per stack:
>
> - **Python:** `verify-green` runs `uv run pytest --cov` + `mypy` + `ruff check` (against scripts/ + hooks/helpers/ — honest scope, not whole-repo). `check-test-exists` blocks edits to `src/*.py` without a sibling `tests/test_<module>.py`. `check-code-quality` runs CQ-001 (mutable globals) + CQ-002 (no-op functions) via AST. Backend-developer / code-reviewer / code-simplifier agents already work.
> - **TypeScript:** `verify-green` runs `npm test` + `tsc --noEmit` + `eslint` (skipping each step cleanly when its config is absent). `check-test-exists` blocks edits to `src/*.ts` and `src/*.tsx` without a sibling `*.test.ts` / `*.spec.ts`. `check-code-quality` blocks empty function bodies (CQ-TS-001). Frontend-developer + frontend-dashboard-refactorer agents are TS-coded.
> - **Go:** `verify-green` runs `go test ./...` + `go vet ./...` + `gofmt -l` + `golangci-lint run` (skips cleanly when golangci-lint not installed). `check-test-exists` blocks edits to `.go` files under `cmd/`, `pkg/`, `internal/` without a sibling `_test.go`. `check-code-quality` blocks empty function bodies (CQ-GO-001). Bindings cite Effective Go + Google Go Style + Uber Go Style + golangci-lint.
> - **Rust:** `verify-green` runs `cargo test --workspace` + `cargo clippy -- -D warnings` + `cargo fmt --check`. `check-test-exists` blocks edits to `src/*.rs` without an in-file `#[cfg(test)]` block OR a corresponding `tests/<module>.rs` integration test. `check-code-quality` blocks empty function bodies (CQ-RS-001) + `.unwrap()` / `.expect()` in production code (CQ-RS-002). Bindings cite Rust API Guidelines + The Rust Book + clippy lints.
> - **Java, Swift, Ruby, Terraform, k8s, markdown:** the profile *architecture* supports them, but no per-language profile content has been authored yet. Operators on these stacks get `/init-project` + DOMAIN.md/PROJECT.md authoring + the agent layer; F020 hooks WARN-skip with a clear stderr line on every unmatched file (no silent gaps).
>
> Trace: see the F020 + F021 rows in the F-feature table below. The audit at `docs/audits/F020-language-coupling-audit.md` lists the 24 surfaces being migrated, the six ADRs at `docs/adrs/F020-001..006` document the load-bearing decisions, and the test fixtures at `tests/fixtures/{typescript,go}-repo/` prove the architecture works across three languages.
...
uv run pytest    # 1000+ contract tests, ~35 seconds
```
...
|---|---|
| **F021** | **TypeScript profile — the F020 proof case on a second language** (shipped 2026-05-16). `standards/code/profiles/typescript/` mirrors the python profile shape: `detection.yaml` (markers: `package.json` + `tsconfig.json`; globs: `*.ts`, `*.tsx`, `*.mts`, `*.cts`; excludes `node_modules`, `dist`, `build`, `.d.ts`, etc.), `README.md`, three rule-binding files (clean-code, error-handling, import-discipline) citing [Google TypeScript Style Guide](https://google.github.io/styleguide/tsguide.html) + [typescript-eslint rules](https://typescript-eslint.io/rules/) + Airbnb JS, and two gate scripts (`verify-green.sh` running `npm test` + `tsc --noEmit` + `eslint`; `check-test-exists.sh` requiring `*.test.ts`/`*.spec.ts` sibling for `src/*.ts` and `src/*.tsx`). 9 new integration tests against `tests/fixtures/typescript-repo/` cover detection, file-routing, exclude_globs, the BLOCK/ALLOW paths, and the WARN-skip path for non-claimed languages. Together with F020, this closes the proof loop: the architecture works for any language that has a community-canonical style guide + linter + test runner. |
| **F020** | **Language-agnostic harness via profile architecture** (foundation shipped 2026-05-16; remainder in progress). Introduces a **profile** primitive at `standards/code/profiles/<profile>/` bundling detection markers, per-rule tool bindings (e.g., ruff codes for python, eslint for typescript), and per-gate executable scripts. New `scripts/detect_profiles.py` walks marker files (`pyproject.toml`, `package.json`, `go.mod`, ...) and writes `.etc_sdlc/profiles.lock`; new `scripts/profile_loader.py` resolves file → profile via fnmatch globs; new `scripts/dispatch_profile.sh` is the centralized bash dispatch helper. Monorepos activate every detected profile, scoped by file path. Files without a matching profile produce stderr WARN + exit 0 (no silent skip, no hard block). Six ADRs at `docs/adrs/F020-001..006`. **Currently profile-aware:** `hooks/verify-green.sh`, `hooks/check-test-exists.sh`, `hooks/check-code-quality.sh`, and four profiles at `standards/code/profiles/{python,typescript,go,rust}/`. **Not yet:** two more hooks (`check-seam-evidence`, `check-completion-discipline`), tier-0 standards Python-vocabulary purge, three agent manifests, SessionStart staleness check, additional profiles (java, swift, terraform, markdown). etc adopts community-canonical style guides ([Google styleguide](https://github.com/google/styleguide), [Kristories/awesome-guidelines](https://github.com/Kristories/awesome-guidelines), PEP 8, Airbnb, Rust API Guidelines, etc.) and never authors its own. See `docs/audits/F020-language-coupling-audit.md` for the originating audit. |
| **F019** | Chief Efficiency Officer — Stop-hook reflection layer per Anthropic's large-codebase guidance. New Stop hook `chief-efficiency-officer.sh` captures every turn end to `.etc_sdlc/efficiency/turn-events.jsonl`, computes active engagement on the current task (sleep gaps > 5 min subtracted, env-tunable via `CEO_IDLE_THRESHOLD_MINUTES`), writes evidence-cited proposals to `.etc_sdlc/efficiency/proposals/`, and updates a rolling daily report at `.etc_sdlc/efficiency/daily/<YYYY-MM-DD>.md`. New PreToolUse hook `sandbox-bypass-tracker.sh` audits every `dangerouslyDisableSandbox: true` invocation. New `/efficiency` skill: `review`, `today`, `baseline`, `mute --until --reason`. Evidence-based ONLY — every observation cites data points + baseline + gap; no hallucinated narrative. Collapses 3 prior queued tasks (stuck-loop detector, operator watchdog, sandbox audit) into one architectural piece. |
| **F018** | Adopt Google's official DESIGN.md spec (Apache 2.0, https://github.com/google-labs-code/design.md) as canonical `/design` output. Impeccable's freeform DESIGN.md → input; Google's spec (YAML frontmatter + canonical Markdown sections) → output. `scripts/design_md_compose.py` extracts hex colors with role-mapping, typography font-family tokens, brand-voice prose (Overview), and anti-references (Do's and Don'ts). Original impeccable file preserved at `DESIGN-impeccable.md`. `npx @google/design.md lint` runs as a best-effort validator. New operator commands: `/design --lint`, `/design --export {tailwind\|css-tailwind\|json-tailwind\|dtcg}`, `/design --refresh`, `/design --spec`. `install.sh` preflight prompts for `@google/design.md` install (interactive only). The tier-0 design preflight hook now also validates DESIGN.md has YAML frontmatter when tier-0 is promoted. AI agents that have been trained on Google's spec now parse etc-produced design systems natively. |
| **F017** | `/journey` skill for SME-led customer-journey capture. Walks a domain expert through 6 plain-English Socratic questions (plus 1 optional emotion question), saves the result to `docs/mvp/journeys/J-NNN-<slug>.md`. Pairs with `/spec` via a new `journey_refs:` field in `state.yaml.spec_phase`. `/build` Step 7.4 fails on empty `journey_refs` UNLESS the spec declares `infrastructure_only: true` + a one-line reason (library upgrade, harness internals, CI tooling). Legacy F001-F016 features pass automatically via a date-check against the F017 release tag. Operator override: `--skip-journey-check="<reason>"` with mandatory non-empty reason logged to verification.md + release-notes.md. Built for non-technical SMEs — forbidden vocabulary includes "acceptance criteria", "stakeholder", "user story"; encouraged vocabulary includes "what they click", "where they get stuck", "how they feel". The intersection of journeys IS the MVP. |
| **F016** | Merge discipline at scale — three carve-outs of the R2-R7 bundle. **R2 (cross-feature collision detection at wave-plan time):** `scripts/cross_feature_collision_check.py` scans every in-flight feature's `files_in_scope` and reports overlaps before /build executes; under `--autonomous` mode the collisions are logged to `state.yaml.build.cross_feature_collisions` and the build proceeds with audit trail. **R3 (Mergiraf preflight):** `install.sh` emits non-blocking INFO if Mergiraf isn't on PATH (semantic merge tool for stacked-PR rebase chains). **R7 (submission/merged schema):** `state.yaml.build.submission` + `state.yaml.build.merged` documented per the Stripe Minions submission-vs-merge distinction (schema only; auto-population deferred). R6 (verification-time return-check / Anthropic Auto Mode pattern) is deferred as its own follow-up. Etc's distinctive market position: no surveyed competitor ships cross-feature scanning. |
| **F015** | `/build` Step 7.5 — spec→ADR coupling gate. After `verification.md` is written and BEFORE the release tag, `scripts/spec_coupling_check.py` scans `spec.md` (and `design.md` if present) for scope-change markers (`deferred`, `removed`, `out-of-scope`, `scope-narrowed`, etc.) **anchored to AC/BR/ADR references** (bare narrative excluded). Each finding must be covered by a decision memo at `.etc_sdlc/features/{slug}/decisions/*.md` OR an ADR appendix at `docs/adrs/*.md` containing "Scope clarification" / "scope-narrowed" / "appendix" + the AC/BR/ADR token. Uncovered → exit 2 BLOCKS release tag write. `--skip-spec-coupling-check="<reason>"` overrides (reason is mandatory, logged to verification.md + release-notes.md). When `etc/feature/F<NNN>/spec/done` tag exists, only flag markers added since; pre-existing markers from original spec are excluded. Sentrux pattern at the spec/ADR layer. |
...
| `scripts/` | Python helpers (`feature_id.py`, `value_hypothesis.py`, `git_tags.py`, `tasks.py`, `sdlc_timing.py`). Called from skills via Bash. `sdlc_timing.py` derives feature-shipping velocity from `feat(F<NNN>):` commit dates — `python3 scripts/sdlc_timing.py [--feature F<NNN>] [--baseline] [--by week] [--since 7d] [--json]`. |
