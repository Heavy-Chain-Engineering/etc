...
**Input:** `docs/lessons-learned-v1.md`, v1 harness agent library, Acme Corp field testing
...
| Fan-out ceiling (~15 agents) | Can't scale to Acme Corp complexity | Managed agent lifecycle with backpressure |
...
**The key innovation:** This does what the human did manually for Acme Corp Round 3. For MVP, this is a Claude API call (the SEM asks Claude to design the topology given the source material inventory). Long-term, it becomes increasingly rule-based as patterns emerge.
...
### The Acme Corp Test
The platform passes its acceptance test when it can execute the Acme Corp Round 3 research plan autonomously:
