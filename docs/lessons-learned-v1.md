...
**Projects tested:** getting-started (SDLC dashboard), Bald Eagle (compliance engine), VenLink (vendor management platform)

## Executive Summary

The v1 coding harness — 23 agents, 8 SDLC phases, 3 hooks, standards, and an SDLC tracker — successfully demonstrated that a structured agent team can execute a full software development lifecycle. The getting-started project was built autonomously in 31 minutes (36 tests, 96% coverage). But real-world testing on Bald Eagle and VenLink exposed fundamental limitations in Claude Code's orchestration model that require a purpose-built platform to overcome.
...
After the OPA/Rego incident (Bald Eagle), the domain fidelity standard — domain briefing documents, mandatory verification step, cascade risk awareness — caught a category of errors that would otherwise propagate through every downstream phase.
...
**Project:** VenLink (Rounds 1 and 2)
...
**Fix applied:** (None in v1) The VenLink research plan included the anti-pattern catalog in agent prompts, but enforcement is still advisory.
...
**What happened:** The VenLink 3-layer research topology was designed and written by the human, not by the SEM. The SEM can execute a simple fan-out but can't assess a 50+ file corpus, classify it across dimensions, and produce a multi-layer research plan.
...
| Bald Eagle | 7/7 | — | — | — | ~2 hrs | 1 (issues found in review) |
| VenLink (research) | Spec (partial) | 16 agents | — | — | ~3 sessions | 3 rounds required |
