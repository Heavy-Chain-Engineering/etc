...
## F-2026-05-29 — /janitor: autonomous harness cleanup (2026-05-29)

Boy-Scout-rule background cleanup skill, built through the full `/spec → /architect → /build` pipeline. Surveys the repo, performs flawless-only cleanup (lint/format, test-proven dead-code, whitespace/EOF/import-order) in a **git worktree cut from `main`**, verifies the harness gates green, runs a mechanical pre-PR boundary check, and opens a PR — structurally unable to touch the operator's working tree (a janitor bug cannot corrupt in-flight work). Two modes: interactive `/janitor` and hands-off `/janitor --autonomous` (zero prompts, for an operator-supplied scheduler). Janitor submits; a human merges — never pushes `origin/main`, never self-merges, never `--no-verify`, never `dangerouslyDisableSandbox`.

Per-category **graduated trust** (`scripts/janitor_trust.py`, sole writer of `.etc_sdlc/janitor/trust.yaml`): every category starts in `preview` (draft PR) and auto-promotes to `autonomous` (ready-for-review PR) after N=5 consecutive clean merges, derived by lazy `gh`+git reconciliation; any non-clean merge resets the streak. The **single-source write boundary** (`standards/process/janitor-write-boundary.md`) is read by both the orchestrator prompt and the mechanical veto (`scripts/janitor_boundary_check.py`, path-canonicalized, no hardcoded copy). The fix-subagent (`agents/janitor.md`) is worktree-scoped — its toolset IS the security boundary (no `gh`, no push, no network). 5 ADRs (worktree-isolation, direct-fix-v1/autochain-v2, lazy-trust-reconciliation, gh-PR-with-local-fallback, N=5 trust). The `/goal → /spec → /architect → /build` autochain is the documented **v2** north star (ADR-002), not built. spec-enforcer COMPLIANT 14/14; +91 tests (trust 99%, boundary 100%, 35 skill-contract, 5 registration).
