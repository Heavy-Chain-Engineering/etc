...
- `test_graph_engine.py` -- Tests for GraphEngine and `build_fanout_graph`: graph CRUD, node scheduling (dependency resolution, ready detection), fan-out/reduce pattern, node lifecycle, graph completion detection.
...
- `test_run_engine.py` -- Tests for RunEngine: run_once cycle, deploy_ready_nodes, check_graph_completions, get_status, get_pending_actions. Mocks `AgentRunner.deploy` to avoid real API calls.
